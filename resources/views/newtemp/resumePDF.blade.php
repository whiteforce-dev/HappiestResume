...
background: linear-gradient(90deg, rgba(33,33,33,1) 0%, rgba(33,33,33,1) 70%, rgba(4,8,9,1) 70%); display:block"></div>
    <div style=" color:#000; padding:30px"> 
      ...
      <h1 style="font-size:35px; margin:0; font-weight:800; text-transform:uppercase">{{ isset($name[0]) ? $name[0] : '' }}</h1>
        ...
        <p style="font-weight:light; text-transform:uppercase; letter-spacing:2px; margin-top:3px">{{ ($title->title) != '' ? $title->title : 'Job Title' }}</p>
      </div>
  ...
  <div style="width:550px; color:#fff; float:left;">
    ...
    ?>
      ...
      <!--Work Experience-->
      <?php
        $path = './public/resume_yellow/assets/imgs/work-experience.png';
        $type = pathinfo($path, PATHINFO_EXTENSION);
        $data = file_get_contents($path);
        $base650 = 'data:image/' . $type . ';base64,' . base64_encode($data);
    ?>
      <div style="background:#040809; width:400px; text-transform:uppercase; font-size:23px; font-weight:300"> <img src="{{ $base650 }}" style="display:inline-block; float:left; width:45px; "/>
        <p style="margin:0px; padding-left:15px; width:265px; text-align:right;  line-height: 44px;">Work experience</p>
      </div>
      <div style="padding:20px 0 30px;line-height:22px "> 
      @if($title->company == '')
      <div style="display:flex; margin:0 -15px">
          <div style="width:30%; padding:0 15px"><strong>2010 - 2020</strong></div>
          <div style="width:70%; padding:0 200px"><strong style="text-transform:uppercase; font-weight:700">JOB TITLE</strong>
      ...
      @else
        ...
        <div style="display:flex; margin:0 -15px">
          <div style="width:30%; padding:0 15px"><strong>{{ date('Y', strtotime($work->start_date)) }} - {{ date('Y', strtotime($work->end_date)) }}</strong></div>
          <div style="width:70%; padding:0 200px"><strong style="text-transform:uppercase; font-weight:700">{{ $work->title }}</strong>
            <p style="font-size:14px; margin:5px 0 30px; line-height:22px">{{ $work->designation }},{{ $work->company }}</p>
