...
background: linear-gradient(90deg, rgba(33,33,33,1) 0%, rgba(33,33,33,1) 70%, rgba(4,8,9,1) 70%); display:block"></div>
    ...
    $user_country = $user_pro->user_country;
    $user_state = $user_pro->user_state;
    $user_city = $user_pro->user_city;
    $cur_title = App\UserProfile::where('user_id',$user_id)->value('current_title');
    ...
    ?>
      <div style="line-height:45px">
        ...
        <p style="font-weight:light; text-transform:uppercase; letter-spacing:2px; margin-top:3px">{{ ($title->designation) != '' ? $title->designation : 'Current Title' }}</p>
      </div>
...
<br>
      ...
      <!--Work Experience-->
      <div style="background:#040809; width:400px; text-transform:uppercase; font-size:23px; font-weight:300"> <img src="{!! asset('public/resume_yellow/assets/imgs/work-experience.png') !!}" style="display:inline-block; float:left; width:45px; "/>
        <p style="margin:0; width:265px; padding:0 20px;     line-height: 44px;display:inline-block; ">Work experience</p>
      </div>
      <div style="padding:20px 0 30px; ">
      @php( $cur_work = App\UserProfile::where('user_id',$user_id)->first())
      @if($title->company != '')

      @foreach($user_work as $work)
      @php($job_city = App\City::where('city_id',$work->job_city)->value('city_name'))
        <div style="display:flex; margin:0 -15px">
        @php($start_date = date('Y', strtotime($work->start_date)))
        @if( $start_date == '' || $start_date == '1970' || $start_date == '0000' || $start_date == null)
        @php($start_date = '')
        @endif
        @php($end_date = date('Y', strtotime($work->end_date)))
        @if( $end_date == '' || $end_date == '1970' || $end_date == '0000' || $end_date == null) 
         @if($start_date != '')
        @php( $end_date = 'Now' )
        @else
        @php( $end_date = '' )
        ...
        @endif
          <div style="width:30%; padding:0 15px"><strong>{{ $start_date }} - {{ $end_date }}</strong></div>
          <div style="width:70%; padding:0 15px"><strong style="text-transform:uppercase; font-weight:700">{{ $work->designation ? $work->designation : 'Job Title' }}</strong>
            <p style="font-size:14px; margin:5px 0 30px; line-height:22px">{{ $work->company }}</p>
            <p style="font-size: 14px;margin: -33px 0 30px;line-height: 22px;">{{ $job_city  }}</p>
          </div>
        </div>
        <br>
      @endforeach
        <!-- <div style="display:flex; margin:0 -15px">
          ...
          <div style="width:30%; padding:0 15px"><strong>2018 - Now</strong></div>
          <div style="width:70%; padding:0 15px"><strong style="text-transform:uppercase; font-weight:700">{{ $cur_work->current_title ? $cur_work->current_title : 'Job Title' }}</strong>
            <p style="font-size:14px; margin:5px 0 30px; line-height:22px">{{ $cur_work->current_comapny }}</p>
      ...
      <div style="display:flex; margin:0 -15px">
          ...
          <div style="width:70%; padding:0 15px"><strong style="text-transform:uppercase; font-weight:700">JOB TITLE</strong>
    ...
    <br>
     ...
     <div style="background:#040809; width:400px; text-transform:uppercase; font-size:23px; font-weight:300"> <img src="{!! asset('public/resume_yellow/assets/imgs/projects.jpg') !!}" style="display:inline-block; float:left; width:45px; "/>
      ...
      <div style="padding:20px 0; ">
        ...
        <p style="margin-top:5px;"><a href="#" style="color:#fff; text-decoration:none; ">{{ $user_app->user_project }}</a></p>
...
<div style="width:550px; color:#fff; float:left;margin-top:40px">
    <div style="padding:30px" >
      ...
      <div style="padding:20px 0; ">
        ...
        <p style="margin-top:5px;"><a href="#" style="color:#fff; text-decoration:none; ">{{ $user_app->user_achievments }}</a></p>
...
<div style="padding: 20px 0 0; position: absolute; width: 900px; bottom: 0;">  <h1 style="letter-spacing:10px; text-align:center; font-weight:900; font-size:65px; margin-top:0; margin-bottom:20px; padding:0 0 10px 0;  -webkit-text-stroke-width: 2px;
